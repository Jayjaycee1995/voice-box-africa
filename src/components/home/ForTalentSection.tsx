...
const ForTalentSection = () => {
  ...
  const staggerDelays = useStaggerAnimation(benefits.length, 100);
      ...
      <div className="container mx-auto px-4 relative">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="order-2 lg:order-1">
            <p className={`text-sm font-medium text-secondary mb-2 transition-all duration-500 ${isVisible ? 'animate-in fade-in slide-in-from-left-4' : 'opacity-0 translate-x-4'}`}>For Talent</p>
            <h2 className={`font-heading text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4 transition-all duration-500 delay-100 ${isVisible ? 'animate-in fade-in slide-in-from-left-4' : 'opacity-0 translate-x-4'}`}>
              Find a great
              <br />
              <span className="gradient-brand-text">opportunity</span>
            </h2>
            <p className={`text-muted-foreground text-lg mb-8 leading-relaxed transition-all duration-500 delay-200 ${isVisible ? 'animate-in fade-in slide-in-from-left-4' : 'opacity-0 translate-x-4'}`}>
            ...
            <div className="grid grid-cols-3 gap-6 mb-10">
              {benefits.map((benefit, index) => (
                <div key={benefit.title} className={`text-center lg:text-left transition-all duration-500 ${isVisible ? 'animate-in fade-in slide-in-from-left-4' : 'opacity-0 translate-x-4'}`} style={{ transitionDelay: isVisible ? `${staggerDelays[index]}ms` : '0ms' }}>
          ...
          {/* Image */}
          <div className={`relative order-1 lg:order-2 transition-all duration-500 delay-300 ${isVisible ? 'animate-in fade-in slide-in-from-right-4' : 'opacity-0 -translate-x-4'}`}>
